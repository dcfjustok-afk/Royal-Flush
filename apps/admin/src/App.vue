...
<template>
  <a class="skip-link" href="#admin-main">跳转到主要内容</a>
  <div v-if="apiMode && authState !== 'authorized'" class="admin-auth-shell">
    ...
    <main id="admin-main" class="admin-auth-main">
      ...
      <form v-else class="admin-login" @submit.prevent="loginAdmin">
        ...
        <label>管理员账号<input v-model.trim="loginAccount" name="admin-account" type="tel" inputmode="numeric" autocomplete="username" placeholder="请输入管理员账号…" spellcheck="false" :disabled="authBusy" /></label>
        <label>管理员密码<input v-model="loginPassword" name="admin-password" type="password" autocomplete="current-password" placeholder="请输入管理员密码…" :disabled="authBusy" /></label>
  ...
  <div v-else class="admin-shell">
    ...
    <main id="admin-main" class="admin-main">
      ...
      <template v-else-if="activeSection === 'users'">
        <section class="admin-workspace"><header class="workspace-tools"><label><Search /><input v-model="search" name="user-search" autocomplete="off" aria-label="搜索用户" placeholder="搜索昵称、手机号或用户 ID…" spellcheck="false" /></label><button class="danger-button" type="button" @click="resetOpen = true"><RotateCcw />重置全站积分</button></header>
