  ...
  <div class="page-shell profile-page">
    ...
    <main class="form-page-content">
      ...
      <div class="profile-grid">
        ...
        <aside class="profile-side"><ScoreAddPanel /><section class="account-facts"><header><h2>账号</h2><CalendarDays /></header><dl><div><dt>显示名称</dt><dd>{{ store.currentUser?.nickname ?? "未登录" }}</dd></div><div><dt>账号 ID</dt><dd>{{ store.currentUser?.id ?? "--" }}</dd></div></dl></section></aside>
      </div>
    </main>
  </div>
</template>
