...
const inviteUrl = computed(() => `${location.origin}/invite/RF-2806`);
...
<template>
  <div class="page-shell waiting-page">
    ...
    <main class="waiting-content">
      <header class="waiting-heading"><div><span class="live-label"><VoiceMeter active />好友正在加入</span><h1>{{ store.roomConfig.name }}</h1><p>RF-2806 · 房主：你</p></div><button class="button light" type="button" @click="copyInvite"><Check v-if="copied" /><Copy v-else />{{ copied ? "已复制" : "复制邀请链接" }}</button></header>
      ...
      <aside class="waiting-console">
        ...
        <button class="ready-button" :class="{ ready }" type="button" @click="ready = !ready"><Check v-if="ready" /><span><strong>{{ ready ? "已准备" : "准备入局" }}</strong><small>{{ ready ? "等待房主开始" : "确认牌局设置和语音状态" }}</small></span></button>
        <button class="button primary wide" type="button" :disabled="!ready" @click="router.push('/rooms/room-saturday/table')"><Play />开始牌局</button>
      </aside>
    </main>
  </div>
</template>
