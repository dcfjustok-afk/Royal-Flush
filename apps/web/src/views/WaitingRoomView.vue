  ...
  <div class="page-shell waiting-page">
    ...
    <main class="waiting-content">
      ...
      <aside class="waiting-console">
        ...
        <section><header><h2>桌内语音</h2><Headphones /></header><button class="voice-test" type="button" :disabled="store.voiceBusy" @click="store.toggleMicrophone"><span :class="{ live: store.microphoneEnabled }"><Mic v-if="store.microphoneEnabled" /><MicOff v-else /></span><span><strong>{{ store.voiceBusy ? "正在连接" : store.microphoneEnabled ? "麦克风正常" : "麦克风已关闭" }}</strong><small>{{ store.voiceError || (store.microphoneEnabled ? "语音已连接" : "点击开启") }}</small></span><VoiceMeter :active="store.microphoneEnabled" /></button><select v-if="store.microphones.length > 1" class="device-select" aria-label="麦克风设备" :value="store.selectedMicrophoneId" @change="selectMicrophone"><option v-for="device in store.microphones" :key="device.deviceId" :value="device.deviceId">{{ device.label }}</option></select></section>
