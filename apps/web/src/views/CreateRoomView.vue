  ...
  <div class="page-shell create-page">
    ...
    <main id="main-content" class="form-page-content">
      ...
      <form class="room-form" @submit.prevent="createRoom">
        <section class="form-section"><header><h2>房间</h2><span>2–8 位好友</span></header><div class="field-grid"><label class="field wide"><span>房间名称</span><input v-model="form.name" name="room-name" autocomplete="off" required maxlength="24" placeholder="输入房间名称…" /></label><label class="field"><span>最多人数</span><select v-model="form.maxPlayers" name="max-players"><option v-for="count in 7" :key="count" :value="count + 1">{{ count + 1 }} 人</option></select></label></div></section>
        <section class="form-section"><header><h2>牌局节奏</h2><span>创建后不可修改</span></header><fieldset><legend>盲注</legend><div class="segmented-control"><label v-for="preset in (['2/5', '5/10', '10/20'] as BlindPreset[])" :key="preset"><input v-model="form.blindPreset" name="blind-preset" type="radio" :value="preset" /><span>{{ preset }}</span></label></div></fieldset><fieldset><legend>行动时间</legend><div class="segmented-control"><label v-for="seconds in ([20, 30, 45] as const)" :key="seconds"><input v-model="form.actionSeconds" name="action-seconds" type="radio" :value="seconds" /><span>{{ seconds }} 秒</span></label></div></fieldset></section>
        <section class="form-section chip-config"><header><div><h2>筹码面额</h2><p>默认面额始终可重复组合</p></div><span><LockKeyhole />创建后锁定</span></header><div class="configured-chips"><span v-for="chip in BASE_CHIP_DENOMINATIONS" :key="chip" class="config-chip fixed">{{ chip }}<small>默认</small></span></div><fieldset><legend>额外大额筹码</legend><div class="large-chip-options"><label v-for="chip in LARGE_CHIP_DENOMINATIONS" :key="chip" :class="{ selected: form.largeChips.includes(chip) }"><input name="large-chips" type="checkbox" :value="chip" :checked="form.largeChips.includes(chip)" @change="toggleLargeChip(chip)" /><span class="config-chip">{{ chip }}</span><strong>{{ form.largeChips.includes(chip) ? "已启用" : "不启用" }}</strong><Check v-if="form.largeChips.includes(chip)" /></label></div></fieldset><div class="chip-preview"><span>牌桌将提供</span><strong>{{ allowedChips.join(" / ") }}</strong></div></section>
        <section class="form-section voice-config"><header><h2>桌内语音</h2><Headphones /></header><label class="toggle-row"><span><strong>开启实时语音</strong><small>不录音、不保存、不转写</small></span><input v-model="form.voiceEnabled" name="voice-enabled" type="checkbox" role="switch" /></label></section>
