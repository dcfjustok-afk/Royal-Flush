...
export const api = {
  ...
  register: (phone: string, password: string, nickname: string) => request<{ user: AccountUser; balance: number }>("/auth/register", { method: "POST", body: JSON.stringify({ phone, password, nickname }) }),
  login: (phone: string, password: string) => request<{ user: AccountUser; balance: number }>("/auth/login", { method: "POST", body: JSON.stringify({ phone, password }) }),
  logout: () => request<{ ok: boolean }>("/auth/logout", { method: "POST", body: "{}" }),
  me: () => request<{ user: AccountUser; balance: number; activeRoomId: string }>("/me"),
  updateMe: (nickname: string) => request<{ user: AccountUser; balance: number; activeRoomId: string }>("/me", { method: "PATCH", body: JSON.stringify({ nickname }) }),
  ...
  voiceToken: (roomId: string) => request<VoiceToken>(`/rooms/${roomId}/voice-token`, { method: "POST", body: "{}" }),
  ...
  voiceWebSocketUrl: (roomId: string) => {
    const configured = import.meta.env.VITE_WS_BASE_URL as string | undefined;
    if (configured) return `${configured.replace(/\/$/, "")}/rooms/${roomId}/voice-events`;
    const protocol = location.protocol === "https:" ? "wss:" : "ws:";
    return `${protocol}//${location.host}${baseUrl}/rooms/${roomId}/voice-events`;
  },
};
